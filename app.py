...
def render_rate_donut(rate: float, color: str, title: str = ""):
    ...
    fig = go.Figure(
        data=[go.Pie(values=[filled, empty], hole=0.7, sort=False, direction="clockwise",
                     marker=dict(colors=[color, "#e5e7eb"]), textinfo="none")]
    )
    fig.update_layout(
        ...
        annotations=[dict(text=f"{rate:.1f}%", x=0.5, y=0.5, showarrow=False,
...
if not df_long_rpt.empty:
    ...
    with col_m1: render_metric_card("🎯", f"{sel_year_rpt}년 계획", f"{fmt_num_safe(total_curr_plan)} {unit_str}", "", COLOR_PLAN)
    with col_m2:
...
if app_mode == "for Sharing":
    ...
    # ── 9. 연도별 산업용 세부 업종 추이 ──
    st.markdown("<hr style='margin: 30px 0;'>", unsafe_allow_html=True)
    st.markdown("#### 📊 9. 연도별 산업용 세부 업종 추이")

    # CSV에서 산업용만 추출
    df_ind_sh = df_csv_tab[df_csv_tab["상품명"].astype(str).str.contains("산업용")].copy() if not df_csv_tab.empty and "상품명" in df_csv_tab.columns else pd.DataFrame()
    if "업종분류" in df_ind_sh.columns and "업종" not in df_ind_sh.columns:
        df_ind_sh["업종"] = df_ind_sh["업종분류"]

    ind_target_cols_sh = ["섬유업종", "펄프업종", "1차금속", "식료품", "기타"]

    def map_industry_name_sh(name):
        name = str(name)
        if "섬유" in name: return "섬유업종"
        if "펄프" in name or "종이" in name: return "펄프업종"
        if "1차" in name and "금속" in name: return "1차금속"
    ...
    # 표시할 연도: 실적 있는 연도 기준 (2020 이상)
    if not df_long_rpt.empty:
        actual_by_year_ind = df_long_rpt[df_long_rpt["계획/실적"] == "실적"].groupby("연")["값"].sum()
        ind_years_sh = sorted([y for y in actual_by_year_ind[actual_by_year_ind > 0].index.tolist() if y >= 2020])
    else:
        ind_years_sh = list(historical_ind_gj_sh.keys())

    if not df_ind_sh.empty and "업종" in df_ind_sh.columns:
        df_ind_sh["단순업종"] = df_ind_sh["업종"].apply(map_industry_name_sh)
        df_ind_sh_filtered = df_ind_sh[df_ind_sh["연_csv"].isin(ind_years_sh)].copy()
    else:
        df_ind_sh_filtered = pd.DataFrame(columns=["연_csv", "단순업종", val_col])

    ind_data_list_sh = []
    for yr in ind_years_sh:
        row_dict = {"연_csv": yr}
        if yr in historical_ind_gj_sh and unit_str == "GJ":
            for tc in ind_target_cols_sh:
                row_dict[tc] = historical_ind_gj_sh[yr].get(tc, 0)
        else:
            yr_df = df_ind_sh_filtered[df_ind_sh_filtered["연_csv"] == yr] if not df_ind_sh_filtered.empty else pd.DataFrame()
            for tc in ind_target_cols_sh:
                v = yr_df[yr_df["단순업종"] == tc][val_col].sum() if not yr_df.empty else 0
                row_dict[tc] = v
        ind_data_list_sh.append(row_dict)

    ind_pivot_sh = pd.DataFrame(ind_data_list_sh).set_index("연_csv")
    ind_pivot_sh = ind_pivot_sh[ind_target_cols_sh]

    ind_stack_full_sh = ind_pivot_sh.reset_index().melt(id_vars=["연_csv"], var_name="단순업종", value_name=val_col)
    yearly_ind_totals_sh = ind_stack_full_sh.groupby("연_csv")[val_col].transform("sum")
    ind_stack_full_sh["비율(%)"] = np.where(yearly_ind_totals_sh > 0, (ind_stack_full_sh[val_col] / yearly_ind_totals_sh * 100).round(1), 0)
    ind_stack_full_sh["텍스트"] = ind_stack_full_sh.apply(
        lambda x: f"{x[val_col]:,.0f}<br>({x['비율(%)']}%)" if x[val_col] > 0 else "", axis=1
    )

    ind_color_map_sh = {
        "섬유업종": "#1B3A6B", "펄프업종": "#2E6FAB",
        "1차금속": "#4A9CC7", "식료품": "#7BBDD4", "기타": "#B0CFE0",
    }
    fig_ind_sh = px.bar(
        ind_stack_full_sh, x="연_csv", y=val_col, color="단순업종",
        title=f"연도별 산업용 세부 업종 판매량 추이 ({unit_str})",
        text="텍스트",
        color_discrete_map=ind_color_map_sh,
        category_orders={"단순업종": ["섬유업종", "펄프업종", "1차금속", "식료품", "기타"]}
    )
    fig_ind_sh.update_layout(
        xaxis_title="연도", yaxis_title=f"판매량 ({unit_str})",
        barmode="stack", margin=dict(t=40, b=20, l=20, r=20),
        xaxis=dict(type='category', linecolor="#DDDDDD"),
        plot_bgcolor="white", paper_bgcolor="white",
    ...
    fig_ind_sh.update_traces(
        textposition='inside', insidetextanchor='middle',
        textfont=dict(size=12, color="white"),
        marker_line_width=0,
    )
    fig_ind_sh.for_each_trace(
        lambda t: t.update(textfont=dict(size=12, color="black")) if t.name == "기타" else None
    )
    st.plotly_chart(fig_ind_sh, use_container_width=True)

    st.markdown(f"**📊 연도별 산업용 구성비 상세 표 ({unit_str})**")
    ind_table_sh = ind_pivot_sh.copy()
    ind_table_sh["💡 총계"] = ind_table_sh.sum(axis=1)
    for col in ind_target_cols_sh:
        ind_table_sh[col] = ind_table_sh.apply(
            lambda row, c=col: f"{row[c]:,.0f} ({(row[c] / row['💡 총계'] * 100):.1f}%)" if row['💡 총계'] > 0 else f"{row[c]:,.0f} (0.0%)", axis=1
        )
    ind_table_sh["💡 총계"] = ind_table_sh["💡 총계"].apply(lambda x: f"{x:,.0f}")
    ind_table_sh = ind_table_sh.reset_index().rename(columns={"연_csv": "연도"})
    st.dataframe(center_style(ind_table_sh.style), use_container_width=True, hide_index=True)

    latest_year_ind_sh = max(ind_years_sh) if ind_years_sh else 2025
    total_latest_ind_sh = ind_pivot_sh.loc[latest_year_ind_sh].sum() if latest_year_ind_sh in ind_pivot_sh.index else 0
    top4_val_sh = sum(
        ind_pivot_sh.loc[latest_year_ind_sh, tc]
        for tc in ["섬유업종", "펄프업종", "1차금속", "식료품"]
        if latest_year_ind_sh in ind_pivot_sh.index and tc in ind_pivot_sh.columns
    )
    top4_ratio_sh = (top4_val_sh / total_latest_ind_sh * 100) if total_latest_ind_sh > 0 else 0
    st.markdown(f"""
    <div style="background-color: #f8f9fa; border-left: 4px solid #1f77b4; padding: 15px; border-radius: 4px; margin-bottom: 40px; color: #1e40af; font-size: 15px;">
        <strong>💡 [산업용 구성 요약]</strong> {latest_year_ind_sh}년 산업용 전체 판매량은 <strong>{total_latest_ind_sh:,.0f} {unit_str}</strong>이며, 주요 4대 업종(섬유, 펄프, 1차금속, 식료품)이 전체의 <strong>{top4_ratio_sh:.1f}%</strong> ({top4_val_sh:,.0f} {unit_str})를 점유하고 있습니다.
    </div>
    """, unsafe_allow_html=True)
