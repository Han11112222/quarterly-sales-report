...
# ─────────────────────────────────────────────────────────
# ✅ 수정된 코멘트 DB 저장/불러오기
#    - load: GitHub에서 먼저 읽고, 실패 시 로컬 fallback
#    - save: 로컬 + GitHub 동시 저장, 실패 시 화면에 에러 표시
# ─────────────────────────────────────────────────────────
    ...
    try:
        if "GITHUB_TOKEN" in st.secrets:
            ...
            try:
                ...
                pass  # GitHub에 파일 없으면 로컬로 fallback
    except Exception:
        pass

    # GitHub 실패 시 로컬 파일 시도
...
def save_comments_db(db_data):
    """로컬 + GitHub 동시 저장, 실패 시 화면에 에러 표시"""
    # 1. 로컬 저장
    try:
        with open(COMMENT_DB_FILE, "w", encoding="utf-8") as f:
